...
4. **Cross-check every lead against our CURRENT live content (mandatory — the point of the library).** The
...
5. **Log it in three places:** (a) a row in the **Mined-videos ledger** below (with its live-door status);
   ...
   **[`docs/content-backlog.md`](../content-backlog.md)** — the release map. (Corroboration-only leads
   don't get a backlog row — but the ledger/Live-door status must still record that the comparison was
   made.) Update the ledger's `→ /sources` / `→ /briefs` columns whenever a primary or brief from it later
   goes live, and mark the backlog row Done.
6. **Verify before anything ships.** A primary from a note is quotable only after `apologia-citations`
   confirms it and it clears `apologia-argument` + `apologia-orthodoxy` — same as everything else.
7. **Deploy** per the repo rule: commit the **note + INDEX row only** (never the transcript) and
...
| Sean McDowell × Rob Bowman · "The Incarnate Christ and His Critics" — **deity of Christ** interview (HANDS) | `dXxVS2B96Ns` | `mcdowell-bowman-incarnate-christ-deity.md` | — | — (**corroboration** of our most-developed area; `hands.html` already built on Bowman & Komoszewski's HANDS) | 2026-07-24 |
| Sean McDowell × Rob Bowman · same book — **live Q&A** (objections to the deity of Christ) | `72LX8_zwiVw` | `mcdowell-bowman-incarnate-christ-deity.md` | — | — (corroboration; **ONE** improvement → backlog: the John 17:22 "shared glory" objection has no deity rebuttal on-site) | 2026-07-24 |
| Sean McDowell × Titus Kennedy · "Archaeology and the People of the Bible" — **10 attested biblical figures** | `-4RwBPrsXVU` | `kennedy-people-of-the-bible-archaeology.md` | — | — (corroboration of `archaeology.html`; **ONE** P3 enrichment → backlog: name concrete cases, esp. Belshazzar/Nabonidus Cylinder) | 2026-07-24 |
| Sean McDowell × Spencer Klavan · "Light of the Mind, Light of the World" — **science, materialism & God** | `-gHic1q-0qg` | `klavan-light-of-the-mind-science-god.md` | — | — (corroboration of the Science & Faith essays; **ONE CANDIDATE** → backlog: the contested quantum-mechanics-→-Mind argument, human decision) | 2026-07-24 |
