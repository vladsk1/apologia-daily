...
| ⭐ **Beginning of the universe — the pre-20th-century history, and who actually declined the kalam move** | **RotGH** (⚠ **ch. 4 pp. 68-73 ONLY**; endnotes unphotographed) | **Aquinas (*ST* I q.46 a.2), Augustine, Maimonides** — affirmed a temporal beginning BY FAITH while holding reason could not decide it (the highest-value lead: `library/kalam.html` never mentions Aquinas); **Bonaventure** contra; **Olbers's paradox**, **Thomas Digges 1576**, **Edgar Allan Poe, *Eureka* (1848 — PUBLIC DOMAIN, a `/sources` candidate)**, **Harlow Shapley** for `library/bigbang.html`. ⚠⚠ **Meyer is the leading ID advocate and this book is the ID cumulative case — the science-tab/ID question is an OPEN OWNER DECISION in `CLAUDE.md`; mine the history, not the design inference, until it is settled.** ⚠ Do not carry the **Simon Singh** motive-claim in our own voice |
