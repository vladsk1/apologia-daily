...
| 2.8 | **An orphan "Coming soon" mastery page is indexed.** | `ev-m-legacy.html` (1.6 KB, `<h1>Coming soon`) is linked from nowhere but is in `search-index.json`. The ev-s8 legacy material now lives in five pages (abolition, compassion, equality, progress, science-history). | Search sends learners to a placeholder. Remove it from the index or redirect it. |
...
4. **Remove the fake Reading Club banner** (2.7) and **de-index `ev-m-legacy.html`** (2.8). *Gate: none.*
