...
- **Research aids you may consult for argument shape + leads (not as quotable sources):** `Grep`/`Read` the public-domain corpus (`sources/*.json`, `sources-index.json`) and the owned-book research maps (`docs/book-research/*.md`, e.g. the Geisler–Turek note). Use them to find the strongest form of an argument, the best objections, and *which primaries to cite* — then quote the **primaries**, verified. Treat every citation in a research map as unverified until you confirm it against the primary; never reproduce a copyrighted book's prose.
