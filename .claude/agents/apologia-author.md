...
- **Research aids you may consult for argument shape + leads (not as quotable sources):** `Grep`/`Read` the public-domain corpus (`sources/*.json`, `sources-index.json`), the owned-book research maps (`docs/book-research/*.md`, e.g. the Geisler–Turek note — start at `book-research/INDEX.md`), and the owned-video research maps (`docs/video-research/*.md` — start at `video-research/INDEX.md`, mined apologetics talks). Assess whichever has the best material for the topic. Use them to find the strongest form of an argument, the best objections, and *which primaries to cite* — then quote the **primaries**, verified. Treat every citation in a research map as unverified until you confirm it against the primary (video transcripts especially mishear names/dates); never reproduce a copyrighted book's prose or a transcript.
