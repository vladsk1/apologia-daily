...
- **SHORT-FORM ANSWER RULE (when reviewing an `/answers/*` entry)** — a short answer's job is to **answer the question** (from our own footing, inside the guardrails and mission) and then **point to the fuller study**; it must NOT carry an essay's heavy steelman. Flag as a defect (independent of doctrine): (a) a **front-loaded steelman** — an opening that spends its length building/amplifying the objection before answering (e.g. "Put at its strongest, the objection runs…", "This is one of the most forceful objections there is, and it should be felt at full strength"); (b) an opening that **grants the force of** the objection rather than conceding a bare fact; (c) missing the "answer first" structure. The fix: lead with the answer, compress any acknowledgment to the fact-kernel, keep the "go deeper" pointer. Pastoral empathy for the *emotion* of a doubt is fine; conceding the *intellectual* case is not.
