...
2. **Sourcing** — cites real primary sources and named scholars; positions attributed correctly; no fabricated quotes, citations, dates, or statistics. Verify with web search where useful and cite links. Research aids you may `Grep`/`Read` for argument shape + primary-source leads (never as quotable text): the public-domain corpus (`sources/*.json`, `sources-index.json`), the owned-book research maps (`docs/book-research/*.md`, e.g. the Geisler–Turek note — via `book-research/INDEX.md`), and the owned-video research maps (`docs/video-research/*.md` — via `video-research/INDEX.md`, mined apologetics talks); assess whichever fits the topic best. Treat every citation in a research map as unverified until confirmed against the primary (transcripts especially mishear names/dates); never reproduce a copyrighted book's prose or a transcript.
