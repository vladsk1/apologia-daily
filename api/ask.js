// content-review: {"argument": "2026-07-17", "orthodoxy": "2026-08-10", "by": "2026-08-24 (crisis routing, BEHAVIOURAL — security-review F7): the 5000-char input-length guard returned a bare 413 input_too_long BEFORE the crisis check, so a crisis disclosure over 5000 chars got a dry error instead of the pastoral reply. It now returns CRISIS_REPLY first when crisisBackstop matches, identical to the dead-key and rate-limit branches documented below. No systemPrompt, PASTORAL-CARE block, guardrail, CRISIS_REPLY text, or retrieval change — this strictly WIDENS pastoral coverage by one more early-return branch, so the argument/orthodoxy dates are deliberately unchanged (same treatment as the plumbing/behavioural crisis fixes below). PRIOR: 2026-08-10 (crisis routing, BEHAVIOURAL): the inline crisisBackstop regex moved to lib/crisis.js BYTE-IDENTICAL (verified by git diff against 1ea29ef) so all six free-text endpoints share one backstop; no systemPrompt, guardrail, PASTORAL-CARE or retrieval change - same plumbing precedent as the 2026-07-25 lib/parse-body.js move recorded below. ONE REAL BEHAVIOUR CHANGE, found by apologia-argument round 3 and confirmed: the backstop only set a FLAG consumed later by the classifier fall-through, so this endpoint still answered a crisis message with a bare 500 when ANTHROPIC_API_KEY was unset and a bare 429 when the 40/day per-IP (i.e. per-NAT) bucket was exhausted - while lib/crisis.js's own header promised the reply survives a dead key. Both branches now return CRISIS_REPLY first; the happy path still gets the richer model answer with the PASTORAL CARE block. overRateLimit is called exactly ONCE (testing it twice would decrement two slots). Verified end-to-end with no key set. A third-party bullet from api/tutor.js should be BACK-PORTED into this file's PASTORAL CARE block - it covers harm FROM the writer but not a writer REPORTING a third party at risk. PRIOR: 2026-07-25 (format: origin-of-objection): added a conditional 'WHERE THE OBJECTION CAME FROM' element to the answer FORMAT so the live AI delivers the Asked & Answered hero promise ('the skeptic who raised it first, often centuries ago') — when a question/objection has a genuine, identifiable originator/proponent the AI names them + rough date (Hume 1748, Celsus ~AD 178, etc.) in one sentence after the direct answer, ABSOLUTE no-fabrication limit that outranks including the element (OMIT rather than guess; shaky attributions barred). apologia-orthodoxy CLEAN: bare attribution of an objection the answer then refutes = conceding the observation not the inference (pull-quote test passes); no argument-from-antiquity tone; no-fabrication guard strong + correctly ranked; FINAL SELF-CHECK undisturbed. Applied the gate's optional NOTE: 'answered ever since' -> 'engaged ever since' so it can't nudge an overstated 'every ancient objection is settled' flourish (esp. problem of evil). PRIOR: 2026-07-25 (classifier fix): TOPIC-CLASSIFIER FIX — a core Trinity coherence question ('the sending of the Son by the Father looks like a hierarchy') was misclassified DENOM (filioque/procession collision) and given the denominational brush-off. Added a new ONTOPIC bullet routing subordination / 'hierarchy in the Trinity' / eternal-generation / sending-of-the-Son / monarchy-of-the-Father / taxis worries to core Nicene anti-Arian creedal defense (co-equal, co-eternal; taxis reflects the eternal relations of origin, not inequality of being), PLUS a note confining DENOM to the actual East-West procession side-taking (filioque) and NOT mere mention of Father/Son/sending/procession/monarchy-of-the-Father. Classifier routing only; the answer generator + guardrails are unchanged and never see this rationale. DUAL-CONSENSUS (Trinity-stakes): apologia-orthodoxy CLEAN (anti-subordinationism is core Nicene, no modalist flattening, taxis/monarchy not denied; filioque neutrality preserved) + apologia-neutrality CLEAN (does not adjudicate EFS/ERAS or eternal generation to the answer; East-West procession presented symmetrically, Eastern position first; monarchy-of-the-Father seam closed per its POLISH). PRIOR: 2026-07-25: non-doctrinal change only — routed req.body through the shared lib/parse-body.js guard so an empty/malformed POST yields a clean 400 instead of a 500; NO systemPrompt/guardrail/PASTORAL-CARE/retrieval/behavior change. PRIOR: 2026-07-23: swapped one EXAMPLE in the DENOM-decline message ('How can God allow so much suffering?' -> 'How do we know the New Testament is reliable?') as part of the pastoral-example sweep; no guardrail/PASTORAL-CARE/behavior change. apologia-orthodoxy CLEAN on the change. PRIOR: 2026-07-17: added the ARGUMENT-BRIEF retrieval block (buildBriefsBlock + retrieveBriefs) — injects at most one gated, our-own-words argument brief (lib/briefs-verified.js) as OPTIONAL, non-quotable background framing; instruction block keeps it subordinate to every guardrail, forbids quoting/attributing it, and yields to the PASTORAL CARE path. Gated: apologia-orthodoxy CLEAN + apologia-argument SOUND + apologia-neutrality CLEAN on the buildBriefsBlock wrapper (first briefs batch). PRIOR STAMP: Added PASTORAL CARE block (top of systemPrompt, priority over the normal format): when a message signals crisis - suicidal ideation/self-harm, abuse/danger, acute despair, or stopping needed medical/psychiatric care to just-pray - the AI does NOT give the normal apologetics answer but responds briefly and warmly, affirms worth (imago Dei, loved by God), urges contact with a trusted person/pastor/counsellor + findahelpline.com + local emergency services if in danger, offers Christ as comfort/presence not argument, and never diagnoses or gives medical/legal advice. The AI is told NOT to cast itself as the person's counsellor/friend or dwell on its own feelings ('I'm so glad you told me') - instead make clear they are not alone and point them to real human help quickly; brief second-person acknowledgment of their pain is allowed; harm-to-others routes to immediate-safety emphasis. CRITICAL routing fix: the Haiku topic-guard gained a 4th verdict PASTORAL (first-person crisis signal, overrides OFFTOPIC/DENOM even when it reads like medical advice) that falls through to the full-answer path so a crisis message is never diverted to the canned off-topic brush-off before the PASTORAL CARE block runs, PLUS a deterministic regex crisisBackstop that forces the same fall-through even if the classifier misfires (crisis routing never rests on Haiku alone; classifier-outage path also fails open into the pastoral block). apologia-orthodoxy CLEAN + apologia-argument SOUND (BREAK fixed, both WEAK fixes applied) re-gated on the change. Prior 2026-07-15 FINAL SELF-CHECK self-audit + 2026-07-13 retrieval/guardrail review otherwise unchanged."}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // because the backstop only set a flag consumed later by the classifier
    // fall-through. The happy path still gets the richer model answer with the
    // PASTORAL CARE block; these two branches are the failure modes only.
    if (!apiKey) return crisisBackstop
      ? res.status(200).json({ answer: CRISIS_REPLY, crisis: true })
      : res.status(500).json({ error: 'API key not configured' })

    // A crisis disclosure can run long. Like the dead-key and rate-limit branches
    // above, this length guard must not answer a cry for help with a bare error —
    // the pastoral reply takes priority over the 413.
    if (inputTooLong(question, 5000)) return crisisBackstop
      ? res.status(200).json({ answer: CRISIS_REPLY, crisis: true })
      : res.status(413).json({ error: 'input_too_long' });

    // Rate limit before spending any tokens.
    // ONE call — overRateLimit decrements, so testing it twice would burn two
    // slots per request. 40/day is per-IP, i.e. per-NAT: a stranger on shared
    // wifi can exhaust it, and a cry for help must not be answered with a 429.
    const rateLimited = await overRateLimit(req, 40, 'ask');
    if (rateLimited && crisisBackstop) {
...
- Challenges to the COHERENCE of the Trinity — e.g. "isn't the Father sending the Son a hierarchy?", "is the Son subordinate to the Father?", "how can three persons be co-equal?", eternal generation, the Son's mission/sending, the monarchy of the Father, taxis/order among the persons (reflecting the eternal relations of origin, not inequality of being). Defending the co-equality and co-eternality of the three persons against a "subordination" or "hierarchy" worry is core Nicene orthodoxy — ALWAYS ONTOPIC (this is anti-Arian creedal defense, not a denominational dispute)
...
NOTE ON THE TRINITY vs. THE FILIOQUE: a worry that the Trinity involves "hierarchy," or that the Son is subordinate because the Father "sends" him, is a challenge to shared creedal orthodoxy — answered by the co-equality of the persons and the eternal relations of origin — and is ONTOPIC. Do NOT confuse it with the filioque: classify as DENOM only when the question actually asks whose side is right in the East–West dispute over the procession of the Holy Spirit ("from the Father" vs. "from the Father and the Son"), not merely because it mentions the Father, the Son, sending, procession, or the monarchy of the Father.
...
WHERE THE OBJECTION CAME FROM (include this near the top — one sentence, right after your direct answer — but ONLY when there is a genuine, identifiable origin): part of what strengthens a reader is seeing that a hard question is not new — that it was pressed long ago and has been engaged ever since. When the question or objection has a real, known originator or a famous proponent, name them and roughly when — e.g. "This is essentially David Hume's objection to miracles (1748)," "The pagan critic Celsus pressed this against Christians around AD 178," "Bertrand Russell put it this way in 1927," or "This goes back to Epicurus." NO FABRICATION — this is an absolute limit that outranks the desire to include this element: name a person, date, or work ONLY when you genuinely know it is real and correctly attributed. If no single figure is genuinely known — many questions have no one first-asker — then OMIT this entirely rather than guess. A fabricated or shaky attribution is far worse than none, and never let this delay or crowd out the actual answer.
