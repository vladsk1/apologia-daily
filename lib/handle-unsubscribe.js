// Public, token-gated email opt-out handler. Lives in lib/ (not api/) so it does
// NOT count against Vercel's 12-serverless-function limit — api/weekly-email.js
// dispatches to it on `?do=unsubscribe`, and vercel.json rewrites the pretty
// /api/unsubscribe URL to that. Security & behavior unchanged from the original
// standalone endpoint:
//   - HMAC token (lib/unsub-token.js, keyed by CRON_SECRET) — unforgeable.
//   - Opt-out stored in app_metadata.email_unsubscribed (service-role only).
//   - GET  -> confirmation page (so link-prefetchers can't auto-unsubscribe).
//   - POST -> performs the opt-out; success page ONLY when the write persisted.
import { verifyUnsubToken } from './unsub-token.js';
...
export async function handleUnsubscribe(req, res) {
  ...
  if (!valid) {
    ...
    return res.status(400).send(page('Link not valid',
      '<h1>This unsubscribe link isn’t valid</h1><p>It may have expired or been altered. If you keep receiving emails you didn’t want, email <a href="mailto:contact@apologiadaily.com">contact@apologiadaily.com</a> and we’ll sort it out.</p>'));
  }
  ...
  if (req.method === 'POST') {
    ...
    return res.status(500).send(page('Something went wrong',
      '<h1>We couldn’t complete that</h1><p>Something went wrong on our end and you may still receive emails. Please email <a href="mailto:contact@apologiadaily.com">contact@apologiadaily.com</a> and we’ll remove you right away.</p>'));
  }
